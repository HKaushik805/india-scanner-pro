...
# --- CSS ---
st.markdown("""<style>.stDeployButton {display:none !important;} footer {visibility: hidden !important;} #MainMenu {visibility: hidden !important;} [data-testid="collapsedControl"] {visibility: visible !important; background-color: #1a73e8 !important; color: white !important; border-radius: 5px !important;} .stButton>button { width: 100%; background-color: #1a73e8; color: white; height: 3.5em; border-radius: 10px; font-weight: bold; border: none;}</style>""", unsafe_allow_html=True)
...
with st.sidebar.expander("✂️ Manual Crop"):
    t_m, b_m, l_m, r_m = st.slider("Top %",0,50,0), st.slider("Bottom %",0,50,0), st.slider("Left %",0,50,0), st.slider("Right %",0,50,0)
    ...
    for u_file in uploaded_files:
        if u_file.name.lower().endswith('.pdf'):
            ...
            for p_n in range(len(doc)):
                with st.spinner(f"PDF Page {p_n+1}"):
                    # OPTIMIZATION: Render at 2x instead of 3x for 2x speed boost
                    pix = doc.load_page(p_n).get_pixmap(matrix=fitz.Matrix(2, 2))
            ...
            with st.spinner("Processing..."):
                res = scan_image(Image.open(u_file), ink_power, do_warp, [t_m, b_m, l_m, r_m], scan_mode, False)
                p_path = os.path.join(TEMP_DIR, f"p_{uuid.uuid4()}.jpg")
                cv2.imwrite(p_path, cv2.cvtColor(res, cv2.COLOR_RGB2BGR) if len(res.shape)==3 else res, [cv2.IMWRITE_JPEG_QUALITY, 95])
                saved_paths.append(p_path)
                with cols[idx%4]: st.image(res, use_container_width=True)
    ...
    if st.sidebar.button("🚀 BUILD FINAL PDF"):
        if saved_paths:
            with st.spinner("Finalizing..."):
                ...
                st.sidebar.download_button("🔥 DOWNLOAD PDF", data=pdf.output(dest='S').encode('latin-1'), file_name="final_scan.pdf", mime="application/pdf")